  ...
  methods: {
    ...
    getTagsRefsIndex(path) {
      if (this.tagsViewList.length > 0) {
        ...
        // 如果没找到，设置为0以避免负数索引
        if (this.tagsRefsIndex < 0) {
          this.tagsRefsIndex = 0;
        }
      } else {
        // 如果列表为空，重置索引
        this.tagsRefsIndex = 0;
    ...
    tagsViewmoveToCurrentTag() {
      this.$nextTick(() => {
        ...
        // 检查索引是否有效
        if (this.tagsRefsIndex < 0 || this.tagsRefsIndex >= tagsRefs.length) return;
        ...
        // 检查元素是否存在
        if (!liFirst || !liLast) return;
        
        // 当前滚动条的值
        let scrollRefs = this.$refs.scrollbarRef && this.$refs.scrollbarRef.$refs && this.$refs.scrollbarRef.$refs.wrap;
        ...
        if (liDom === liFirst) {
          ...
          else if (liPrevTag) beforePrevL = liPrevTag.offsetLeft - 5;
          if (liIndex === liLength) afterNextL = liLast.offsetLeft + liLast.offsetWidth + 5;
          else if (liNextTag) afterNextL = liNextTag.offsetLeft + liNextTag.offsetWidth + 5;
    ...
    // 更新滚动条显示
    updateScrollbar() {
      if (this.$refs.scrollbarRef && this.$refs.scrollbarRef.update) {
